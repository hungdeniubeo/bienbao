...
if __name__ == "__main__":
    ...
    while True:
        ...
        for rect in rects:
            ...
            crop_img = np.asarray(img[y1:y2, x1:x2])  # Cắt vùng ảnh chứa biển báo
            crop_img = cv2.resize(crop_img, (32, 32))  # Resize về (32,32)
            crop_img = preprocessing(crop_img)  # Tiền xử lý ảnh (grayscale + equalize + normalize)

            # Đảm bảo ảnh có đúng kích thước (1,32,32,1)
            crop_img = np.expand_dims(crop_img, axis=-1)  # Thêm kênh màu (1 channel grayscale)
            crop_img = np.expand_dims(crop_img, axis=0)  # Thêm batch size (1 ảnh)

            cv2.imshow("afterprocessing", crop_img.squeeze())  # Hiển thị ảnh sau xử lý

            predictions = model.predict(crop_img)  # Dự đoán biển báo
            classIndex = np.argmax(predictions, axis=1)  # Lấy class có xác suất cao nhất
